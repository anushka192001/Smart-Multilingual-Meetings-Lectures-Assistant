...
def transcribe_with_srt(audio_path: str):
    """
    Transcribe audio with whisper model, then write SRT with sentence timestamps.
    """
    # Load the Whisper model (can use "base", "small", "medium", or "large")
    model = whisper.load_model("base")  # or "small", "medium", "large", "large-v2", etc.

    # Transcribe your audio
    result = model.transcribe(audio_path, verbose=False)  # Add language="en" if you want to force language

    # result is a dict: result['text'] = full transcript, result['segments'] = time-stamped segments
    ...
    if full_transcript.strip() == "":
        raise RuntimeError("Transcription failed: No transcript generated by Whisper.")

    # For time-aligned segments (optional, if you need timeline or chatbot context)
    for segment in result["segments"]:
        start = segment["start"]  # in seconds
        end = segment["end"]      # in seconds
        text = segment["text"]
        start_ts = ms_to_srt_time(start)
        end_ts = ms_to_srt_time(end)
        timelines.append(f"{start_ts} --> {end_ts}  **{text.strip()}**")
        transcripts.append(text.strip())
        chatbot_timelines.append(f"{start_ts}-->{end_ts}*{text.strip()}*")
